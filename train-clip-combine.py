...
model_fine_tune = Unet(
    dim = 64,
    dim_mults = (1, 2, 4, 8)
).cuda()
diffusion_fine_tune = GaussianDiffusion(
    model_fine_tune,
    image_size = 128,
    timesteps = 1000,   # number of steps
    loss_type = 'l1'    # L1 or L2
...
features_source=torch.from_numpy(np.load('draw/features-real_img.npy')).cuda().mean(0)
...
diffusion_fine_tune.load_state_dict(torch.load('results/fine-tune-model/sketches/models/401.pth'))
...
for epoch in range(100):
    for batch_idx,batch in enumerate(train_dataloader):
        ...
        if global_step%100==0 and global_step!=0:
            ...
            sampled_images1 = diffusion_target.ddim_sample((16, 3, 128, 128), sample_step=25,max_step=1000,min_step=opts.noise_step)
            sampled_images = diffusion_fine_tune.ddim_sample((16, 3, 128, 128), sample_step=25, max_step=opts.noise_step,
            ...
            #sampled_images,sampled_middle_images = diffusion_target.ddim_sample(x.shape, sample_step=25,return_middle=True)
            sampled_images1,sampled_middle_images1 = diffusion_target.ddim_sample(x.shape, sample_step=25, max_step=1000,
                                                           min_step=opts.noise_step,return_middle=True)
            sampled_images,sampled_middle_images2 = diffusion_fine_tune.ddim_sample(x.shape, sample_step=25,
                                                             max_step=opts.noise_step,
                                                             min_step=-1, start_img=sampled_images1,return_middle=True)
