...
ddp_model_source = DDP(model_source, device_ids=[local_rank], output_device=local_rank)
ddp_model_target = DDP(model_target, device_ids=[local_rank], output_device=local_rank)
ddp_diffusion_source = DDP(diffusion_source, device_ids=[local_rank], output_device=local_rank)
ddp_diffusion_target = DDP(diffusion_target, device_ids=[local_rank], output_device=local_rank)
...
# for i in ckpt.keys():
#     print(i)
# ddp_diffusion_source.load_state_dict(torch.load('pretrained_models/481157.pth'))
# ddp_diffusion_target.load_state_dict(torch.load('pretrained_models/481157.pth'))
...
if opts.resume_iter!=-1:
    ddp_diffusion_target.load_state_dict(torch.load(os.path.join(save_dir,'models','%d.pth'%opts.resume_iter)))
...
for epoch in range(100):
    for batch_idx,batch in enumerate(train_dataloader):
        if batch_idx%100==0:
            print(batch_idx)
        image=batch.cuda()
        t,(x,loss_diffusion) = ddp_diffusion_target.few_shot_forward(image)
        loss=loss_diffusion
        if global_step<=100 or (global_step>100 and global_step%1==0) :
            ...
            with torch.no_grad():
                ...
                t, (x, _) = ddp_diffusion_source.few_shot_forward(real_image,step=opts.noise_step)
                #x_start_source = (diffusion_source.batch_p_sample(x, t, None)+1)/2
                feature_source=clip_model.encode_img(real_image)
            x_start_target = (ddp_diffusion_target.batch_p_sample(x, t, None)+1)/2
        ...
        if global_step%50==0 and global_step!=0:
            ...
            # sampled_images = diffusion_target.sample(batch_size = 24)
                sampled_images = ddp_diffusion_target.ddim_sample((16, 3, 128, 128), sample_step=50)
            ...
            noises = ddp_diffusion_target.p_losses(real_image, t, return_x=True)
            sampled_images,sampled_middle_images = ddp_diffusion_target.ddim_sample(x.shape, sample_step=50,return_middle=True,start_img=noises, max_step=noise_step,
                                                                        min_step=-1)
        ...
        if global_step % 100 == 0 and global_step!=0:
            torch.save(ddp_diffusion_target.state_dict(), os.path.join(save_dir,'models/%d.pth'%global_step))
        global_step += 1
