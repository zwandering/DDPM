...
#diffusion.load_state_dict(torch.load('/home/huteng/DDPM2/results/cartoon/clip_center_loss/step=300,beta=10/models/500.pth'))
...
with torch.no_grad():
    for batch_idx,imgs0 in enumerate(real_dataloader):
        ...
        for step in tqdm(reversed(range(0, noise_step)), desc = 'sampling loop time step', total =noise_step):
            ...
            img, x_start = diffusion.p_sample(img, step, self_cond,loss_fn=None,feature_merge=tmp_img)
            if step%10==0:
                ...
                #save_image(img, 'results/%d.jpg'%step,normalize=True)
        img = (img+1)*0.5
