...
import utils
#DDPM扩充训练集训练训练 先去除风格损失较远的图像，然后去除与真实对应样本内容损失大的图像
parser = ArgumentParser()
parser.add_argument('--save_dir', type=str,default='results', help='Path to experiment output directory')
parser.add_argument('--double_diffusion', action='store_true',default=False, help='Path to experiment output directory')
...
def adjust_images2(imgs,delete_num=10):
    ...
    new_imgs = torch.stack(new_imgs, dim=0)
    return new_imgs
def adjust_images1(imgs,ori_imgs,delete_num=10,save_img=False):
    l=len(imgs)
    style_losses = torch.zeros((imgs.size(0), real_imgs.size(0))).cuda()
    for i in range(len(real_imgs)):
        tmp_img = real_imgs[i:i + 1]
    ...
    new_ori_imgs=[]
    for i in range(len(imgs)):
        if i not in style_idx[:int(delete_num//1.3) ]:
            new_imgs.append(imgs[i])
...
class Train_Data(Dataset):
    def __init__(self, real_imgs,add_imgs,epoch=0):
        repeat_num=1
        if add_imgs.size(0)/real_imgs.size(0)>4:
            repeat_num=(add_imgs.size(0)/real_imgs.size(0))//4+1
        imgs=torch.cat([real_imgs.repeat(int(repeat_num),1,1,1),add_imgs],dim=0)
...
diffusion0.load_state_dict(torch.load('pretrained-models/481157.pth'))
diffusion_ema.eval()
utils.accumulate(diffusion_ema, diffusion, 0)
...
for epoch in range(100000):
    ...
    for batch_idx,batch in enumerate(train_dataloader):
        ...
        # loss.backward()
        # optizer.step()
        diffusion_ema_module = diffusion_ema
        utils.accumulate(diffusion_ema_module, diffusion, accum)
        global_step+=1
    with torch.no_grad():
        if not opts.double_diffusion:
            sampled_images = diffusion_ema.ddim_sample((sample_size, 3, 128, 128),sample_step=25)
            ...
            noises = diffusion_ema.p_losses(test_imgs, t,return_x=True)
        else:
            noises = torch.randn((sample_size, 3, 128, 128), device='cuda')
            step = 1000
            test_imgs = diffusion0.ddim_sample((sample_size, 3, 128, 128), sample_step=25, start_step=step,
                                                       start_img=noises.clone())
            print(test_imgs.min(),test_imgs.max())
        sampled_images = diffusion_ema.ddim_sample((sample_size, 3, 128, 128), sample_step=25,start_step=step,start_img=noises)
        ...
        torch.save(diffusion_ema.state_dict(), '%s/models/%d.pth' % (opts.save_dir, epoch))
        save_image(test_imgs, '%s/images/%d-sample-ori.jpg' % (opts.save_dir, epoch), nrow=sample_size // 5,normalize=False)
        save_image(sampled_images, '%s/images/%d-sample-train.jpg' % (opts.save_dir, epoch), nrow=sample_size//5,normalize=False)
        sampled_images,sampled_ori_images=adjust_images1(sampled_images,test_imgs,delete_num=sample_size-8,save_img=True)
        ...
        add_imgs,ori_imgs=adjust_images1(add_imgs,ori_imgs,delete_num=4)
