...
from criteria.encoders.model_irse import Backbone
...
class IDLoss(nn.Module):
    ...
    def encode_img(self,y,cropped=False):
        if not cropped:
            if y.size(3)!=256:
                y=self.resize(y)
        y_feats = self.extract_feats(y,cropped)  # Otherwise use the feature from there
        return y_feats
    def forward(self,y,cropped=False):
        if not cropped:
            if y.size(3)!=256:
                y=self.resize(y)
        y_feats = self.extract_feats(y,cropped)  # Otherwise use the feature from there
        return y_feats
    def forward2(self, y_hat, y,cropped=False):
        if not cropped:
            if y_hat.size(3)!=256:
