...
model_source.prepare(two_stage_step=opts.noise_step,style_condition=True)
...
model_target.prepare(two_stage_step=opts.noise_step,style_condition=True)
...
#style_img_and_features=[style_imgs,style_features]
...
# diffusion_source.load_state_dict(load_condition_model('/home/huteng/DDPM2/pretrained_models/ffhq/400000.pth'),strict=False)
# diffusion_target.load_state_dict(load_condition_model('/home/huteng/DDPM2/pretrained_models/ffhq/400000.pth'),strict=False)
diffusion_source.load_state_dict(load_condition_model('/home/huteng/DDPM2/results2/sketches/clip_center_loss/step=300,beta_f=0,beta_s=0/models/3000.pth'),strict=True)
diffusion_target.load_state_dict(load_condition_model('/home/huteng/DDPM2/results2/sketches/clip_center_loss/step=300,beta_f=0,beta_s=0/models/3000.pth'),strict=True)
...
optimizer = Adam(diffusion_target.model.parameters(), lr = 1e-4, betas =(0.9, 0.99))
    ...
    for batch_idx,batch in enumerate(train_dataloader):
        ...
        # if global_step<=100 or (global_step>100 and global_step%3==0) :
            ...
            with torch.no_grad():
                ...
                t, (x, _) = diffusion_source.few_shot_forward(real_image,step=opts.noise_step,x_self_cond=real_image_vgg_features)
                #x_start_source = (diffusion_source.batch_p_sample(x, t, None)+1)/2
                feature_source=clip_model.encode_img(real_image)
            x_start_target = (diffusion_target.batch_p_sample(x, t, x_self_cond=real_image_vgg_features)+1)/2
            ...
            if opts.beta_filter!=0:
                x_start_target_gray=x_start_target.mean(dim=1).unsqueeze(1).repeat(1,3,1,1)
                real_image_gray=real_image.mean(dim=1).unsqueeze(1).repeat(1,3,1,1)
                loss_filter=mse_loss_reduce(vgg.features(x_start_target_gray),vgg.features(real_image_gray))*opts.beta_filter
                # loss_filter=(resize_right.resize(resize_right.resize(x_start_target_gray, 1 / filter_N), filter_N)- resize_right.resize(
                #     resize_right.resize(real_image_gray, 1 / filter_N), filter_N))**2
                # loss_filter=loss_filter.mean(-1).mean(-1)
                # loss_filter=loss_filter*opts.beta_filter
        ...
        if global_step%50==0 and global_step!=0:
            ...
            sampled_images = diffusion_target.ddim_sample((batch_size, 3, 128, 128), sample_step=50,condition=real_image_vgg_features)
