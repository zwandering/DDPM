...
class GaussianDiffusion(nn.Module):
    ...
    def model_predictions(self, x, t, x_self_cond = None, clip_x_start = False,feature_merge=None):
        model_output = self.model(x, t, x_self_cond,feature_merge=feature_merge)
    ...
    @torch.no_grad()
    def p_mean_variance(self, x, t, x_self_cond = None, clip_denoised = True,feature_merge=None):
        preds = self.model_predictions(x, t, x_self_cond,feature_merge=feature_merge)
    ...
    def p_sample(self, x, t: int, x_self_cond = None, clip_denoised = True,loss_fn=None,feature_merge=None):
        b, *_, device = *x.shape, x.device
        batched_times = torch.full((x.shape[0],), t, device = x.device, dtype = torch.long)
        model_mean, model_variance, model_log_variance, x_start = self.p_mean_variance(x = x, t = batched_times, x_self_cond = x_self_cond, clip_denoised = clip_denoised,feature_merge=feature_merge)
    ...
    @torch.no_grad()
    def ddim_sample(self, shape, clip_denoised = True,loss_fn=None,sample_step=None,max_step=None,min_step=None,start_img=None,return_middle=False,condition=None):
    ...
    def p_losses(self, x_start, t, noise = None,return_x=False,x_self_cond=None):
        ...
        return loss.mean()
    def few_shot_p_losses(self, x_start, t, noise = None,x_self_cond=None):
        b, c, h, w = x_start.shape
        noise = default(noise, lambda: torch.randn_like(x_start))
    ...
    def few_shot_forward(self,img, *args,step=0,t=None, **kwargs):
        b, c, h, w, device, img_size, = *img.shape, img.device, self.image_size
        assert h == img_size and w == img_size, f'height and width of image must be {img_size}'
        # t = torch.randint(600, self.num_timesteps, (1,), device=device).long().repeat(b)
        if t is None:
            t = torch.randint(step, 900, (b,), device=device).long()
        # t=torch.ones_like(t).cuda().long()*900
        img = normalize_to_neg_one_to_one(img)
        return t,self.few_shot_p_losses(img, t, *args, **kwargs)
    def batch_p_mean_variance(self, x, t, x_self_cond = None, clip_denoised = True):
        ...
        model_mean, posterior_variance, posterior_log_variance = self.q_posterior(x_start = x_start, x_t = x, t = t)
        return model_mean, posterior_variance, posterior_log_variance, x_start
    def batch_p_sample(self, x, t, x_self_cond = None, clip_denoised = True,loss_fn=None):
        b, *_, device = *x.shape, x.device
        batched_times = t
